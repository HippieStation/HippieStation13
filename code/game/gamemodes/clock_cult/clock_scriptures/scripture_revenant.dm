...
/datum/clockwork_scripture/invoke_sevtug/check_special_requirements()
	if(!slab.no_cost && clockwork_generals_invoked["sevtug"] > world.time)
		...
		to_chat(invoker, "<span class='warning'>Sevtug has already been invoked recently! You must wait several minutes before calling upon the Formless Pariah.</span>")
		return FALSE
	if(!slab.no_cost && ratvar_awakens)
		...
		to_chat(invoker, "<span class='warning'>Sevtug will not grant his power while Ratvar's dwarfs his own!</span>")
		return FALSE
	return TRUE
...
/datum/clockwork_scripture/invoke_nzcrentr/check_special_requirements()
	if(!slab.no_cost && clockwork_generals_invoked["nzcrentr"] > world.time)
		...
		to_chat(invoker, "<span class='warning'>Nzcrentr has already been invoked recently! You must wait several minutes before calling upon the Eternal Thunderbolt.</span>")
		return FALSE
	return TRUE
