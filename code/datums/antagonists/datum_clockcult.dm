...
/datum/antagonist/clockcultist/give_to_body(mob/living/new_body)
	if(!silent_update)
		if(issilicon(new_body))
			to_chat(new_body, "<span class='heavy_brass'>You are unable to compute this truth. Your vision glows a brilliant yellow, and all at once it comes to you. Ratvar, the Clockwork Justiciar, lies in exile, derelict and forgotten in an unseen realm.</span>")
		else
	...
	if(!silent_update && new_body)
		if(.)
			...
			to_chat(new_body, "<span class='heavy_brass'>Assist your new companions in their righteous efforts. Your goal is theirs, and theirs yours. You serve the Clockwork Justiciar above all else. Perform his every whim without hesitation.</span>")
		else
