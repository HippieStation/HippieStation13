...
/obj/item/weapon/reagent_containers/glass/afterattack(obj/target, mob/user, proximity)
	if((!proximity) || !check_allowed_items(target,target_self=1)) return
	...
	else if(istype(target, /obj/structure/reagent_dispensers) && !target.is_open_container()) //A dispenser. Transfer FROM it TO us.
		...
		if(reagents.total_volume)
			to_chat(user, "<span class='notice'>Doing that would be useless.</span>")
		else
			to_chat(user, "<span class='notice'>You plunge [src] in the [target].</span>")
